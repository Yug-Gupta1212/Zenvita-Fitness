...
import { n as getSafeLocalStorage, t as getSafeDocument } from "./browser-safe-DPMYmjS3.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/AppShell-5p7JScr3.js
...
function AppShell({ title, subtitle, children, hideHeader = false }) {
	...
	(0, import_react.useEffect)(() => {
		...
		if (!storage) return;
		const initialTheme = storage.getItem("zenvita-theme") || "dark";
	...
	const setTheme = (newTheme) => {
		...
		const storage = getSafeLocalStorage();
		if (storage) storage.setItem("zenvita-theme", newTheme);
		applyTheme(newTheme);
	};
